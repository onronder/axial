/**
 * Next.js Middleware - Production-Grade Session Management
 * 
 ...
 * 
 * IMPORTANT: This middleware runs on EVERY request. Keep it fast and fail-safe.
 * 
...
if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
    ...
    // Don't throw - middleware should fail gracefully
}
...
/**
 * Check if a path should be completely excluded from middleware processing.
 */
...
/**
 * Check if a path is a static asset that should skip middleware.
 */
...
export async function middleware(request: NextRequest) {
    ...
    // ---------------------------------------------------------------------
    // 0. FAIL-SAFE: If Supabase is not configured, skip middleware
    // ---------------------------------------------------------------------
    if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
    ...
    try {
        ...
        if (error) {
            ...
            if (!isPublicPath(pathname)) {
                console.warn('[Middleware] Auth error:', error.message, 'Path:', pathname)
            }
            ...
            console.error('[Middleware] Non-session auth error:', error)
        }
        ...
        // IMPORTANT: Don't break the app on middleware errors
...
export const config = {
    matcher: [
         ...
         * 
         * Note: We handle additional exclusions in the middleware function
