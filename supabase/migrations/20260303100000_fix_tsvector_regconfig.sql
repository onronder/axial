-- Migration: Fix to_tsvector/plainto_tsquery regconfig cast
--
-- The previous migration (20260303000000_fix_chat_search.sql) declared
-- search_language as TEXT. PostgreSQL's to_tsvector() and plainto_tsquery()
-- require the first argument to be regconfig, not text. String literals are
-- implicitly cast, but TEXT variables are not — causing SQLSTATE 42883 at
-- runtime. This migration re-creates both functions with explicit
-- search_language::regconfig casts.
