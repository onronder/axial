-- Migration: Update ingest RPC to track file size
-- Supports accurate quota usage calculation
-- Created: 2025-12-27

-- ============================================================
-- UPDATE: ingest_document_with_chunks
-- ============================================================
-- Add p_file_size_bytes parameter to track storage usage

-- Drop old function signature (6 parameters) to replace with new (7 parameters)
...
CREATE OR REPLACE FUNCTION public.ingest_document_with_chunks(
    ...
    p_metadata JSONB,
    p_chunks JSONB,
    p_file_size_bytes BIGINT DEFAULT 0  -- NEW: Track file size for quotas
) RETURNS UUID 
LANGUAGE plpgsql 
SECURITY DEFINER
