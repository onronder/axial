-- =============================================================================
-- Migration: Activate per-language FTS regconfig for ingest and query
-- Created: 2026-04-08
--
-- Purpose:
-- - Generate document_chunks.content_search with per-language regconfig on the
--   active Ghost Protocol RPC path
-- - Preserve ISO 639-1 language metadata writes
-- - Remove the query-side override that forced every search to `simple`
-- - Keep safe fallbacks to `simple` for NULL / empty / unsupported callers
-- =============================================================================

BEGIN;

-- =============================================================================
-- 1. Ghost Protocol ingest RPCs: generate TSVECTOR with caller-provided regconfig
-- =============================================================================
...
CREATE FUNCTION public.ingest_document_chunk(
    ...
    p_language_regconfig TEXT DEFAULT 'simple'
)
RETURNS UUID
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    INSERT INTO document_chunks (
        ...
        p_content_encrypted,
        to_tsvector(
            COALESCE(NULLIF(BTRIM(p_language_regconfig), ''), 'simple')::regconfig,
            COALESCE(p_content_plaintext, '')
        ),
...
BEGIN
    ...
    SELECT
        ...
        to_tsvector(
            COALESCE(NULLIF(BTRIM(chunk->>'language_regconfig'), ''), 'simple')::regconfig,
...
-- =============================================================================
-- 2. hybrid_search: keep caller-provided search_language instead of overriding it
-- =============================================================================
...
-- =============================================================================
-- 3. hybrid_search_scoped: same change with scope filter support
-- =============================================================================
...
GRANT EXECUTE ON FUNCTION public.hybrid_search(TEXT, VECTOR, INT, UUID, FLOAT, FLOAT, FLOAT, TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION public.hybrid_search(TEXT, VECTOR, INT, UUID, FLOAT, FLOAT, FLOAT, TEXT) TO service_role;
