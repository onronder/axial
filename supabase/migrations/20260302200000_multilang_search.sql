-- Migration: Add multilingual full-text search support (H6)
-- Adds language column to document_chunks and parameterizes hybrid_search
-- PostgreSQL lacks native Turkish stemming, so Turkish uses 'simple' config

BEGIN;

-- Add language column to document_chunks (default 'english' for backward compat)
...
BEGIN
    ...
    keyword_results AS (
        SELECT
            ...
            d.metadata,
            ts_rank_cd(
                to_tsvector(COALESCE(search_language, 'simple'), dc.content),
                plainto_tsquery(COALESCE(search_language, 'simple'), query_text),
                32
            )::FLOAT as score,
            ROW_NUMBER() OVER (
                ORDER BY ts_rank_cd(
        ...
        WHERE (filter_org_id IS NULL OR d.organization_id = filter_org_id)
          ...
          AND COALESCE(d.metadata->>'type', '') != 'identity_card'
          AND COALESCE(lower(d.metadata->>'is_identity'), 'false') != 'true'
          AND to_tsvector(COALESCE(search_language, 'simple'), dc.content)
              @@ plainto_tsquery(COALESCE(search_language, 'simple'), query_text)
        ORDER BY score DESC
        LIMIT match_count * 3
    ),
