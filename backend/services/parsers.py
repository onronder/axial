...
try:
    from core.metrics import (
        docx_ocr_fallback_total,
        image_ocr_total,
        llamaparse_fallback_total,
        pdf_scan_detection_total,
    )
except Exception:
    ...
    def process(self, content: bytes, filename: str) -> ProcessedDocument:
        ...
        for i, chunk_text in enumerate(raw_chunks):
            ...
            if token_count > 2000:
                ...
                for _j, sub_text in enumerate(sub_chunks):
    ...
    def _process_with_tesseract(self, content: bytes, filename: str) -> ProcessedDocument:
        ...
        try:
            ...
            if image.mode in ('RGBA', 'P', 'LA'):
                ...
                image = background
            elif image.mode == 'L' or image.mode not in ('RGB',):
                image = image.convert('RGB')
...
class DocumentProcessorFactory:
    ...
    def process(
        ...
        if not processor_class:
            ...
            if mime_type and (mime_type.startswith("text/") or mime_type in cls.TEXT_MIME_TYPES) or not cls._looks_like_binary(content):
