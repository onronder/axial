...
class SubscriptionService:
    ...
    def verify_signature(
        ...
        try:
            ...
            if msg_id:
                msg_a = f"{msg_id}.{timestamp}.".encode() + payload
                messages_to_try.append(msg_a)

            # Format B: timestamp.payload (Common fallback)
            msg_b = f"{timestamp}.".encode() + payload
    ...
    async def handle_webhook(self, event_data: dict[str, Any]):
        ...
        try:
            ...
            elif event_type == "subscription.updated" or event_type == "subscription.active" or event_type == "subscription.uncanceled":
