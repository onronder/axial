...
async def create_checkout_session(data: CheckoutRequest, current_user: str = Depends(get_current_user)):
    if not settings.POLAR_ACCESS_TOKEN:
        raise HTTPException(500, "Billing not configured")
    
    # current_user is a string ID from get_current_user
    team_member = await team_service.get_user_team_member(current_user)
    if not team_member: raise HTTPException(400, "No team found")
