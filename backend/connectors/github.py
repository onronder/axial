...
from connectors.utils import build_config_from_kwargs, resolve_oauth_credentials
...
class CodeFileFilter:
    ...
    def should_include(self, path: str, size: int | None = None) -> bool:
        ...
        return bool(ext and ext in self.EXTENSION_WHITELIST)
...
class NavigationContext:
    ...
    repo: str | None = None       # "owner/repo" if inside a repo
    path: str | None = None       # Path within repo (e.g., "src/components")
    branch: str | None = None     # Branch to use


def parse_parent_id(parent_id: str | None) -> NavigationContext:
    """
