...
class TestIngestConnectorTask:
    ...
    def test_ingest_connector_task_processes_documents(self):
        ...
        with patch("worker.tasks.get_connector", return_value=connector), \
             patch("worker.tasks.DocumentProcessorFactory.process", return_value=parse_result), \
             patch("worker.tasks.generate_embeddings_batch_sync", return_value=[[0.1]]):
            result = tasks.ingest_connector_task(
                user_id="user-1",
                job_id="job-1",
                connector_type="web",
                item_id="item-1",
