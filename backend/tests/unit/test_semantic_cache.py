...
class TestSemanticCache:
    ...
    async def test_cache_miss_returns_none(self, cache):
        ...
        with patch.object(cache, "_get_redis", new_callable=AsyncMock, return_value=mock_redis):
            result = await cache.get([0.1] * 1536, ["scope1"], "org-1")
    ...
    async def test_cache_put_stores_with_ttl(self, cache):
        ...
        assert stored["faithfulness_warning"] == "Needs review"
    ...
    def test_cache_key_includes_scope(self, cache):
        ...
        key1 = cache._cache_key([0.1] * 1536, ["scope1"], "org-1")
        key2 = cache._cache_key([0.1] * 1536, ["scope2"], "org-1")
        assert key1 != key2
    ...
    def test_cache_key_same_for_same_inputs(self, cache):
        ...
        key1 = cache._cache_key([0.1] * 1536, ["scope1"], "org-1")
        key2 = cache._cache_key([0.1] * 1536, ["scope1"], "org-1")
        assert key1 == key2

    @pytest.mark.unit
    def test_cache_key_differs_for_different_orgs(self, cache):
        """Same query/scope must not collide across organizations."""
        key1 = cache._cache_key([0.1] * 1536, ["scope1"], "org-1")
        key2 = cache._cache_key([0.1] * 1536, ["scope1"], "org-2")
        assert key1 != key2

    @pytest.mark.unit
    def test_cache_key_differs_for_allowed_scope_sets(self, cache):
        """Same query/scope must not collide across different effective access sets."""
        key1 = cache._cache_key([0.1] * 1536, ["scope1"], "org-1", ["scope1", "scope2"])
        key2 = cache._cache_key([0.1] * 1536, ["scope1"], "org-1", ["scope1"])
        assert key1 != key2

    @pytest.mark.unit
    def test_cache_key_distinguishes_unrestricted_from_empty_allowed_scopes(self, cache):
        """None (full access) and [] (no access/fail-closed) must not collide."""
        key1 = cache._cache_key([0.1] * 1536, ["scope1"], "org-1", None)
        key2 = cache._cache_key([0.1] * 1536, ["scope1"], "org-1", [])
        assert key1 != key2
    ...
    def test_cache_key_global_scope(self, cache):
        ...
        key = cache._cache_key([0.1] * 10, [], "org-1")
        assert "__global__" in cache._cache_key.__code__.co_consts or key  # key exists
        key2 = cache._cache_key([0.1] * 10, ["scope1"], "org-1")
        assert key != key2
